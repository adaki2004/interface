    ...
    async function fetchL1Reserves() {
      ...
      try {
        ...
        const reservePromises = validAddresses.map(async (address) => {
          ...
          // Only use pairs that don't throw an error when fetching reserves
          return pair.getReserves().catch((error: any) => {
            console.error(`Failed to fetch reserves for address ${address}:`, error)
            return null
          })
        })
        ...
        setReserves(successfulResults.map(result => ({ result, loading: false })))
        setError(null)
      } catch (err) {
